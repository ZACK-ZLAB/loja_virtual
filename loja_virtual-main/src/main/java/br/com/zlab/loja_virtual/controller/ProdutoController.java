...
public class ProdutoController {
	...
	public ResponseEntity<Produto> salvarAcesso(@RequestBody @Valid Produto produto) throws LojaVirtualException, MessagingException, IOException { /*Recebe o JSON e converte pra Objeto*/
		
		
		if (produto.getTipoUnidade() == null || produto.getTipoUnidade().trim().isEmpty()) {
			throw new LojaVirtualException("Tipo da unidade deve ser informada");
		}
		
		if (produto.getNome().length() < 10) {
			throw new LojaVirtualException("Nome do produto deve ter mais de 10 letras.");
		}
		...
		if (produto.getId() == null) {
		  ...
		  if (!produtos.isEmpty()) {
			  throw new LojaVirtualException("Já existe Produto com a descrição: " + produto.getNome());
		  }
		}
		
		
		if (produto.getCategoriaProduto() == null || produto.getCategoriaProduto().getId() <= 0) {
			throw new LojaVirtualException("Categoria deve ser informada");
		}
		
		
		if (produto.getMarcaProduto() == null || produto.getMarcaProduto().getId() <= 0) {
			throw new LojaVirtualException("Marca deve ser informada");
		}
		
		if (produto.getQtdEstoque() < 1) {
			throw new LojaVirtualException("O produto dever ter no minímo 1 no estoque.");
		}
		
		
		
		if (produto.getImagens() == null || produto.getImagens().isEmpty() || produto.getImagens().size() == 0) {
			throw new LojaVirtualException("Deve ser informado imagens para o produto.");
		}
	...
	public ResponseEntity<Produto> obterAcesso(@PathVariable("id") Long id) throws LojaVirtualException { 
		
		Produto produto = produtoRepository.findById(id).orElse(null);
		
		if (produto == null) {
			throw new LojaVirtualException("Não encontrou Produto com código: " + id);
		}
